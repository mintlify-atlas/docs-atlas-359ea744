---
title: "Color, Color32, Rect, and IntRect Types"
sidebarTitle: "Colors & Rects"
description: "Float RGBA Color and byte RGBA Color32 types, named color constants in Colors, plus Rect and IntRect for 2D rectangle math."
---

Prowl.Vector provides two complementary color types and two rectangle types for 2D spatial work. `Color` stores RGBA channels as `float` values in the **[0, 1]** range, making it the standard type for shader parameters, material properties, and color math. `Color32` stores the same four channels as `byte` values in the **[0, 255]** range, matching the format of raw pixel data and packed GPU textures. The static `Colors` class is a namespace anchor for W3C-standard named color constants defined directly on `Color`. `Rect` and `IntRect` represent axis-aligned bounding rectangles with float and integer precision respectively, and share a rich set of spatial query methods.
...
The W3C-standard named colors are defined as static properties directly on the `Color` struct. The `Colors` static class in `Prowl.Vector` serves as the namespace anchor for this standard, while the actual constants are accessible via `Color.*`:
...
`Rect` represents an axis-aligned rectangle in 2D float space. It stores two `Float2` corners: `Min` (bottom-left) and `Max` (top-right). The constructor always normalizes the corners so `Min ≤ Max` in both axes.
